/**
 * Created by sunharuka on 14-7-23.
 */
...
function CarDAO(db) {
  ...
  if (false === (this instanceof CarDAO)) {
    console.log('Warning: CarDAO constructor called without "new" operator');
    return new CarDAO(db);
  }

  var posts = db.collection("cars");
  ...
  this.incrementLikes = function(permalink, comment_ordinal, callback) {
    ...
    var updateBoby = {};
    updateBoby["comments."+comment_ordinal+".num_likes"]=1;

    posts.update({permalink:permalink},{"$inc":updateBoby},function(err,numModified){
      callback(err,numModified);
    });
